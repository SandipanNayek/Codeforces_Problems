...
bool check(ll mid,vector<int>&a,vector<int>&b){
 ll mintime=0;
 for(int i=0;i<a.size();i++){
    if(a[i] > mid && b[i] > mid) return false;
    else if (b[i] <= mid && a[i] > mid) mintime += b[i]; 
        else if (b[i] <= mid && a[i] <= mid) mintime += 0;
 }
 if(mintime>mid) return false;
 if(mintime <=mid) return true;
}

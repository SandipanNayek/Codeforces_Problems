...
// ------------------------- SOLVE --------------------------
long long myFloor(double x) {
    long long i = (long long)x;
    if (x < 0 && x != i) i--;
    return i;
}

long long myCeil(double x) {
    long long i = (long long)x;
    if (x > 0 && x != i) i++;
    return i;
}
...
void solve(){
    ...
    ll mn=x;
    for(int i=0;i<min(n,60LL);i++) mn/=2;
    for(int i=0;i<min(m,60LL);i++) mn=(mn+1)/2;

    ll mx=x;
    for(int i=0;i<min(m,60LL);i++) mx=(mx+1)/2;
    for(int i=0;i<min(n,60LL);i++) mx/=2;
